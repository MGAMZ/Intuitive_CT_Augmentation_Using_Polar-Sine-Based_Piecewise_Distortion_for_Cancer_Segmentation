...
def tabulate_print(result: AVG_LOSS) -> None:
    ...
    for model_name, loss_dict in result.items():
        row = [model_name]+ [f"{loss_dict[loss_name][0]:.2f}±{loss_dict[loss_name][1]:.2f}"
...
def csv_save(result, csv_save_path:str):
    ...
    for col in df_mean_std.columns:
        df_mean_std[col] = df_mean_std[col].apply(lambda x: f"{x[0]:.2f}±{x[1]:.2f}")
    # Convert each item from (mean, std) to mean
    for col in df_mean.columns:
        df_mean[col] = df_mean[col].apply(lambda x: f'{x[0]:.2f}')
    # Convert each item from (mean, std) to std
    for col in df_std.columns:
        df_std[col] = df_std[col].apply(lambda x: f'{x[1]:.2f}')
...
def parse_args():
    ...
    parser.add_argument('--workdir_root', help='The root path of workdir', type=str, default=r"D:\PostGraduate\DL\mgam_CT\mmseg\work_dirs")
    parser.add_argument('--csv_save_path', help='The path to save csv file', type=str, default=r"D:\PostGraduate\DL\mgam_CT\result\mmseg")
    ...
    parser.add_argument('--loss_name', help='The list of loss name', nargs="+", type=str, default=['mIoU', 'mDice', 'mRecall', 'mPrecision'])
...
def summarize(args):
    ...
    reduction = get_reduction_func(args)

    tsb_list_all_model = get_tensorboard_filelist(args.workdir_root, args.model_list, exp=args.exp_name)
    exec_param = []
    model_name_list = []
    for model_name, tsb_list in tsb_list_all_model.items():
        for tsb_file in tsb_list:
            exec_param.append((tsb_file, reduction, args.loss_name))
    ...
    csv_save(result, os.path.join(args.csv_save_path, f"{args.exp_name}.csv"))
    print(f"CSV文件已保存至：{args.csv_save_path}")
...
if __name__ == '__main__':
    ...
    for exp in args.exp_name:
        ...
        summarize(sub_args) # 启动实验
        print(f"\n{sub_args.exp_name} 统计完成\n")
    
    print(f"\n{args.exp_name} 已全部统计完成\n")
